...
const variantMap: Record<NonNullable<ProductVisualProps["variant"]>, { frame: string; image: string }> = {
  hero: {
    ...
    image: "max-h-[520px] scale-[1.22] rotate-[-2deg]",
  },
  soft: {
    frame:
      "rounded-[2rem] bg-gradient-to-br from-[#fff8fb] via-[#f9f4ef] to-[#f2ebe5] p-6",
    image: "max-h-[460px] scale-[1.1]",
  },
  spotlight: {
    frame:
  ...
  dark: {
    ...
    image: "max-h-[470px] scale-[1.14] drop-shadow-[0_20px_40px_rgba(0,0,0,0.25)]",
  },
};
...
export function ProductVisual({ variant = "soft", className = "" }: ProductVisualProps) {
  ...
  return (
    <div className={`ambient-sheen relative overflow-hidden ${config.frame} ${className}`}>
      ...
      <div className="relative z-10 flex min-h-full items-start justify-center pt-6">
        <Image
          ...
          className={`mx-auto h-[88%] w-auto object-contain animate-float-soft ${config.image}`}
        />
      </div>
    </div>
  );
