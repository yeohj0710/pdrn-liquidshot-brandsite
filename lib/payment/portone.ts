...
function hasInicisVerificationEnv() {
  return Boolean(readRequiredEnv("PORTONE_V1_KEY") && readRequiredEnv("PORTONE_V1_SECRET"));
}

function hasPortOneV2VerificationEnv() {
  return Boolean(readRequiredEnv("PORTONE_V2_SECRET"));
}

export function hasVerificationConfig(paymentMethod: string) {
  if (paymentMethod === INICIS_PAYMENT_METHOD) {
    return hasInicisVerificationEnv();
  }
  return hasPortOneV2VerificationEnv();
}
